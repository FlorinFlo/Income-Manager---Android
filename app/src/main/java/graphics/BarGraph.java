...
public class BarGraph extends View implements HoloGraphAnimate {
	...
	private long mValueStringUpdateInterval = 200;// ms; how often to updateTextFromDatePicker the
													// value strings when
													// animating
	...
	public BarGraph(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mAxisColor = a
				.getColor(R.styleable.BarGraph_barAxisColor, Color.LTGRAY);
		mShowAxis = a.getBoolean(R.styleable.BarGraph_barShowAxis, true);
		mShowAxisLabel = a.getBoolean(R.styleable.BarGraph_barShowAxisLabel,
				true);
	...
	public void onDraw(Canvas canvas) {
		...
		for (final Bar bar : mBars) { // calculate total widths of bars
		...
		for (final Bar bar : mBars) {
			...
			if (mShowBarText) {
				...
				// Check cache to see if we've calculated value text size
				// for bars that aren't being inserted/deleted.
				// Not 100% accurate with non monospace fonts but close
				// enough
				...
				} // for bars inserting/deleting, do the math everytime
					// without a cache.
				else
					while (mPaint.measureText(bar.getValueString()) > boundRight
							- boundLeft) {
